...
export class ClusterManager extends EventEmitter {
    ...
    constructor(options: ClusterManagerOptions) {
        ...
        if (!options.file) throw new Error('CLIENT_INVALID_OPTION | No File specified.');
        if (!path.isAbsolute(options.file)) this.file = path.resolve(process.cwd(), options.file);
